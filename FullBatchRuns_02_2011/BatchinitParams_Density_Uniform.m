function [p,xm,tm,mem,xc,yc] = BatchinitParams_Density_Uniform(chickBoxLength,vfunc,S0);
	
	% Nc = [g1,g2] gives the number of chickens in each group.
	% S0 is the source emission (dimensional). I will want to set it to zero to test my preictions.
	...
	p.Tf = 2000; 			% nondimensional time span, the maximum number of mosquito decisions (simulation can be stopped early if all mosquitoes leave the domain)
	...
	p.chickBoxLength = chickBoxLength/p.L0;
	xc = p.chickBoxCenter(1) + [ -p.chickBoxLength, 0, p.chickBoxLength].';
	xc = [xc;xc;xc];
	yc1 = (p.chickBoxCenter(2) -p.chickBoxLength)*ones(3,1);
	yc2 = p.chickBoxCenter(2)*ones(3,1);
	yc3 = (p.chickBoxCenter(2) + p.chickBoxLength)*ones(3,1);
	yc = [yc1;yc2;yc3];
	...
	p.entrytimerange = [549.6, 550.4]; %when to insert the mosquitoes (normal distribution around this range) 
	Xint = p.L*[0,1];
	dx = Xint(2)-Xint(1);    
	...
	p.tGraph_prior = 0;%100;	% controls how often to graph output before mosqs appear 

function [p,ym,angmem] = BatchsetParams(p,windmode,basedir,fnamestring,chicknum,iternum);
	...
	if exist('iternum','var');
    	p.savefname = fullfile(basedir,[WindName(windmode),'_',fnamestring,int2str(chicknum),'_iternum',int2str(iternum),'.mat']);   
	else;
	   	p.savefname = fullfile(basedir,[WindName(windmode),'_',fnamestring,int2str(chicknum),'.mat']);   
	end    

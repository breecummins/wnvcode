function [mx my whichchicken G_old theta outsideradius inradius mxkeep mykeep mxind myind] = MoveMosqSample(C,s,index,G_old,theta,mx,my,vx,vy,inradius,whichchicken,mxkeep,mykeep,mxind,myind,mosqcount)
...
for k = 1:length(mx)
    ...
    if strcmp(mosqcount,'1')
        mxind(end+1) = mx(k); myind(end+1) = my(k);
    else
        mxind = []; myind = [];
    end
...
dG_mosq = abs(G-G_old(outsideradius));
direction = (dG_mosq >= s.Cthresh);

% higher CO2 concentration results in lower velocity
mspeed = s.S_max - (s.S_max-s.S_min).* ResponseFunction(dG_mosq,s.Cthresh,s.Csat,s.kappa);

function g = AdvectC(xg,yg,C,Cxp,Cxm,Cyp,Cym,U,V,h,Vmag);

% CALCULATE SHIFTED X AND Y VELOCITIES
Uxp = [U(:,2:end),V1Parametric(xg(:,end)+h,yg(:,end),Vmag)];
Uxm = [V1Parametric(xg(:,1)-h,yg(:,1),Vmag), U(:,1:end-1)];
Vyp = [V(2:end,:); V2Parametric(xg(end,:),yg(end,:)+h,Vmag)];
Vym = [V2Parametric(xg(1,:),yg(1,:)-h,Vmag); V(1:end-1,:)];

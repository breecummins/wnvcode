function [s C] = Variable_struct_mps(chickencount,littlechicken,windtype,...
    mosqcount,windsense,pretime,loadname,tstop,time,day)
...
% gridpoints for CO2 including outer strips
s.xZ = s.x0-(s.h/2):s.h:s.xf+(s.h/2);
s.yZ = s.y0-(s.h/2):s.h:s.yf+(s.h/2);
[XZ YZ] = meshgrid(s.xZ,s.yZ);
s.XZ = XZ;
s.YZ = YZ;
%-------------------------------------------------------------------------%
...
if chickencount == 10
    switch littlechicken
        case '2'
            s.Sx = (s.xf-s.x0).*[.06 .06 .06 .06 .08 .08 .08 .08 .10 .93];
            s.Sy = (s.yf-s.y0).*[.75 .79 .83 .87 .75 .79 .83 .87 .81 .81];
        case '1'
...
% note that if the old method of calculating s.dt from s.dt0 is used, this
% will have to be adjusted so that s.tf is calculated before s.dt
% time parameters, in seconds
s.t0 = gtn(end) + gdt;
if isempty(tstop) == 1, s.tf = 720*60;
else s.tf = (s.dt*tstop) + s.t0;
end
...
switch windtype
    ...
    case 'cc'
        ...
        s.windy = @(X,Y) .0005*(s.yf/s.xf)*(-cos(pi*(X/s.xf)).*sin(pi*(Y/s.yf)));
    case 'dp'
        s.d = 0.25;  % chicken "size"
        % calculations in Matrix.m to vary wind between runs
    case 'no'
        s.windx = 0*X;
        s.windy = 0*Y;
end

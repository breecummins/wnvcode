function [p,xm,ym,tm,mem,xc,yc,Cmem,angmem,modevec] = BatchinitParams_Density_ManyPlumes(BoxLengthPerChick,dwidth,dlength,initradius,groupflag,usemosq,inittime,finaltime,S0);
	...
	elseif groupflag == 4;
		...
		p.chickBoxCenter = [200, 50]; % [x center, y bottom] for one group of chickens. 
	...
	tm = inittime*ones(1,nummodes*p.Nm)/p.T; %CHANGED
	if p.Nm > 0;
		disp(['Mosquito insertion time is at t=',num2str(tm(1)),' in nondiml units.'])
	end
	...
	xm0 =  -p.initradius + 2*p.initradius*rand(p.Nm,1); %initial radial position of mosquitoes
	ym0 =  -p.initradius + 2*p.initradius*rand(p.Nm,1); %initial radial position of mosquitoes

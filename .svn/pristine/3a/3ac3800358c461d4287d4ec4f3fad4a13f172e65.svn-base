function [p,xm,ym,tm,mem,xc,yc,Cmem,angmem,modevec] = BatchinitParams_Density_ManyPlumes(BoxLengthPerChick,vfunc,dwidth,dlength,initradius,groupflag,usemosq,inittime,S0);
	...
	p.Tf = 5; 			% nondimensional time span, the maximum number of mosquito decisions (simulation can be stopped early if all mosquitoes leave the domain)
	...
	tm = 1500*ones(1,nummodes*p.Nm); %CHANGED
	if inittime == 0;
		tm = 0*tm;
		disp('Mosquito insertion time is at t=0.')
	end
	p.initradius = initradius/p.L0; %CHANGED all initial position stuff
	thetam = 2*pi*rand(p.Nm,1); %angular position of entering mosquitoes

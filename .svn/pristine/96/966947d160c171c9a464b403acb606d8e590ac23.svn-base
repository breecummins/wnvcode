function [SpatialDistX,SpatialDistY,recordscaling]=SSExploreMosquitoBehavior6_rule5(ruleparams,fixedvars)
...
% maxDmem = fixedvars.maxG *( (ruleparams.maxspd+ruleparams.minspd)/2 )*fixedvars.dt; %total guess
...
for t = 0 : fixedvars.dt : fixedvars.Tf;
	...
	if ~isempty(method);
		...
		recordscaling(end+1) = max(abs(dif/maxDmem));

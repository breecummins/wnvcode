...
pretime = input('Amount of time for CO2 growth (min)?  ');
...
fprintf('release time %1.0f minutes\n',pretime)
end
fprintf('\n')

movieask = input('Visual:  display = d / movie = m / avi = a / none = n ?  ','s');
savename = [condition,' ',windtype,' ',num2str(pretime),'min',' ',num2str(runcount),'r'];

...
for t = 0 : dt : Tf;
	...
	[Cxmi,Cxpi,Cymi,Cypi] = BCPeriodic(Cperiodic);
	
    % CALCULATE DIFFUSION
	Dn = (Cxpn+Cxmn+Cypn+Cymn-4*Cneuman)/h^2;
	Di = (Cxpi+Cxmi+Cypi+Cymi-4*Cperiodic)/h^2;
	...
	if length(u) > 1;
		...
		Flxxi = (Cperiodic.*lupp + Cxpi.*lupm).*up - (Cxmi.*lump + Cperiodic.*lumm).*um;
	else;
		Flxxn = (Cneuman - Cymn).*u;
		Flxxi = (Cperiodic - Cymi).*u;
	end
	Flxyn = (Cneuman- Cymn).*v; %good only for positive v flows
	Flxyi = (Cperiodic- Cymi).*v;
	An = (Flxxn+Flxyn)/h;
	Ai = (Flxxi+Flxyi)/h;
	
	% SUM ALL OF THE TERMS.
    Cneuman = Cneuman + nu*dt*Dn - dt*An; 		
    Cperiodic = Cperiodic + nu*dt*Di - dt*Ai; 		
    ...
    if any(Cperiodic < 0);
        Cmn=min(min(CPeriodic));
        disp(['Periodic BCs, Cmn = ',num2str(Cmn),' at t = ',num2str(t)])
    end
	...
	if mod(t,0.01) < dt/2;
		...
		subplot(122)
		pcolor(xg,yg,Cperiodic)
		shading interp
		caxis([0,cmx])
		title(['Periodic BCs,Time=',num2str(t)])
		colorbar('SouthOutside')

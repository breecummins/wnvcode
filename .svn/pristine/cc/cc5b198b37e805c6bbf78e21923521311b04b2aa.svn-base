function [C,tn,dt,S,Sx,Sy,K,windx,windy,h,X,Y,mx,my,S_max,S_min,b0,bsat,...
    ...
    dCm,dCM] =...
    Variable_m(chickencount,littlechicken,windtype,mosqcount,windsense)
...
if chickencount == 10
    switch littlechicken
        case '2'
            Sx = (xf-x0).*[.03 .03 .03 .03 .05 .05 .05 .05 .07 .96];
            Sy = (yf-y0).*[.78 .82 .86 .90 .78 .82 .86 .90 .84 .84];
        case '1'
            Sx = (xf-x0).*[.93 .95 .95 .95 .95 .97 .97 .97 .97 .04];
            Sy = (yf-y0).*[.84 .78 .82 .86 .90 .78 .82 .86 .90 .84];
    end
elseif chickencount == 9
    switch littlechicken
        case '2'
            Sx = (xf-x0).*[.03 .03 .03 .03 .05 .05 .05 .05 .96];
            Sy = (yf-y0).*[.78 .82 .86 .90 .78 .82 .86 .90 .84];
        case '1'
            Sx = (xf-x0).*[.95 .95 .95 .95 .97 .97 .97 .97 .04];
            Sy = (yf-y0).*[.78 .82 .86 .90 .78 .82 .86 .90 .84];
    end
end
...
switch windtype
    case 'clockwise'
        windx = @(X,Y) .0005*(-sin(pi*(X/xf)).*cos(pi*(Y/yf)));
        windy = @(X,Y) .0005*(yf/xf)*(cos(pi*(X/xf)).*sin(pi*(Y/yf)));

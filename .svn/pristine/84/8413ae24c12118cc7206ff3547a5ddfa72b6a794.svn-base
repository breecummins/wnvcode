function [p,xm,tm,mem,xc,yc,Cmem] = BatchinitParams_2groups(Nc,vfunc,S0);
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	p.Nc = Nc;			% number of chickens split between two groups
	p.tChick = 10;		% controls how often to move the chickens (> mosquito decision time = 1)
						% currently stationary
	p.chickBoxCenter = [p.L/4,  p.L/2; 3*p.L/4, p.L/2]; 
						% [x center, y center; x center, y center] for two groups of chickens. 
	p.chickBoxLength = sqrt(9.29*Nc);
	p.minChickDist = 0.5*sqrt(9.29);
	...
	p.tGraph_prior = 0;%10;	% controls how often to graph output before mosqs appear 

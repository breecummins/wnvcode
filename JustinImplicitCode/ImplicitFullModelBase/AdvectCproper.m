function g = AdvectCproper(xg,yg,C,Cxp,Cxm,Cyp,Cym,U,V,h,Vmag,vfunc,t);

% CALCULATE SHIFTED X AND Y VELOCITIES
Uxp = [U(:,2:end),eval(['V1Parametric',vfunc,'(xg(:,end)+h,yg(:,end),Vmag)'])];
Uxm = [eval(['V1Parametric',vfunc,'(xg(:,1)-h,yg(:,1),Vmag)']), U(:,1:end-1)];
Vyp = [V(2:end,:); eval(['V2Parametric',vfunc,'(xg(end,:),yg(end,:)+h,Vmag)'])];
Vym = [eval(['V2Parametric',vfunc,'(xg(1,:),yg(1,:)-h,Vmag)']); V(1:end-1,:)];

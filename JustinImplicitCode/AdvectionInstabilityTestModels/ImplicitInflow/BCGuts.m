...
for t = 0 : dt : Tf;
    ...
    Cneuman = DiffuseC_implicit_exact_grad(Cneuman,dt,h,nu,1e-8,t);
    Cinflow = DiffuseC_implicit_inflow(Cinflow,dt,h,nu,1e-8,t,LBind,RBind);
	
	% SUM ALL OF THE TERMS.
    Cneuman = Cneuman - dt*An; 		
    ...
    if any(Cinflow < 0);
        Cmn=min(min(Cinflow));
        disp(['Inflow BCs, Implicit Iterate, Cmn = ',num2str(Cmn),' at t = ',num2str(t)])
    end
	...
	if mod(t,0.01) < dt/2;
		...
		shading interp
		caxis([0,cmx])
		title(['Neumann BC,Time=',num2str(t)])
		...
		shading interp
		caxis([0,cmx])
		title(['Inflow BC,Time=',num2str(t)])
		colorbar('SouthOutside')

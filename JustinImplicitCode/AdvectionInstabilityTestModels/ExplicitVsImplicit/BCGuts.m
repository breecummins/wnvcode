...
for t = 0 : dt : Tf;
	...
	[Cxmi,Cxpi,Cymi,Cypi] = BCNeumann(Cinflow);
	
    % CALCULATE DIFFUSION FOR EXPLICIT CASE
    ...
    % CALCULATE DIFFUSION FOR IMPLICIT CASE
    Cinflow = DiffuseC_implicit_exact_grad(Cinflow,dt,h,nu,1e-8,t);
	
	% SUM ALL OF THE TERMS.
    Cneuman = Cneuman + nu*dt*Dn - dt*An; 		
    Cinflow = Cinflow - dt*Ai; 		
    
    % REPORT NEGATIVE CO2 (BUG CHECK)
    if any(Cneuman < 0);
        Cmn=min(min(Cneuman));
        disp(['Neumann BCs, Explicit Iterate, Cmn = ',num2str(Cmn),' at t = ',num2str(t)])
    end
    if any(Cinflow < 0);
        Cmn=min(min(Cinflow));
        disp(['Neumann BCs, Implicit Iterate, Cmn = ',num2str(Cmn),' at t = ',num2str(t)])
    end
	...
	if mod(t,0.01) < dt/2;
		...
		shading interp
		caxis([0,cmx])
		title(['Explicit Iterate,Time=',num2str(t)])
		...
		shading interp
		caxis([0,cmx])
		title(['Implicit Iterate,Time=',num2str(t)])
		colorbar('SouthOutside')
		
		set(gcf,'PaperPosition',[0,0,8.0,6.0])
		set(gcf,'PaperSize',[8.0,6.0])
		saveas(1,['Visualization/',fname,'_',sprintf('%03d',ctr),'.png'])
		ctr=ctr+1;
		% pause(0.5)
	end
end

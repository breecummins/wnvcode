function [inputs, outputs, rmbase, rmhigh, rmlow, data0, datah, datal] = calcsensitivityindex(varname,windmode,basedir);
	...
	SI = mean(inputs(2,:)).*(rmhigh(end,:)-rmlow(end,:))./mean(vardif)./rmbase(end,:); % center difference scaled by the base input and output
	SIplus = mean(inputs(2,:)).*(rmhigh(end,:)-rmbase(end,:))./(mean(vardif)/2)./rmbase(end,:);
	SIminus = mean(inputs(2,:)).*(rmbase(end,:)-rmlow(end,:))./(mean(vardif)/2)./rmbase(end,:);
